...
class RQuery {
	...
	function match($attr, $regexp) {
		return $this->attr($attr, new MongoDB\BSON\Regex($regexp));
	}
	 ...
	 */
	function getOptions() {
		$option = ['projection'=>$this->_results];
		if ($this->_offset >= 0) {
			$option['skip'] = $this->_offset;
		}
		if ($this->_limit > 0) {
			$option['limit'] = $this->_limit;
		}
		if ($this->_sort) {
			$option['sort'] = $this->_sort;
		}
		if (!empty($this->_hints)) {
			$hints = [];
			//TODO 如果这里在UI上勾选了多个索引，该如何处理
			foreach ($this->_hints[0] as $idx=>$hint) {
				$hints[$idx] = $hint;
			}
			$option['hint'] = $hints;
		}
	...
	function cursor() {
		...
		return $this->_collection->find($where, $option);
	}
	...
	function explain() {
		$where = $this->criteria();
		$option = $this->getOptions();
		if(count($option['projection']) == 0) unset($option['projection']);
		$findOpt = new MongoDB\Operation\Find(
			$this->_collection->getDatabaseName(),
			$this->_collection->getCollectionName(),
			$where, $option
		);
		return $this->_collection->explain($findOpt);
	}
	...
	function count($withLimit = false) {
		$where = $this->criteria();
		$option = $this->getOptions();
		return $this->_collection->count($where, $option);
	}
	...
	function upsert(array $obj) {
		return $this->_collection->updateMany($this->criteria(), $obj, array( "upsert" => true, "multiple" => true));
	}
	...
	function batchInsert(array $array) {
		return $this->_collection->insertMany($array);
	}
	/**
	 * 当前操作的集合
	 *
