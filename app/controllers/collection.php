...
class CollectionController extends BaseController {
	...
	public function doIndex() {
		...
		foreach ($indexRet as $index => $indexField) {
			...
			$idxInfo["keystring"] = json_encode($indexField->getKey());
			$indexFields[$index] = $idxInfo;
		}
		...
		$newobj = xn("newobj");
		$newobj = trim($newobj == NULL ? '' : $newobj);
		if (!$newobj) {
			if ($format == "array") {
				x("newobj", 'array(
	"$set" => array (
	...
	"$set": {
		...
		if (empty($criteria)) {
			...
			$row = null;
			$q = formatJsonQuery($criteria);
			$j = json_decode(formatJsonQuery($criteria), true);
			if (isset($j['_id']) && substr($j['_id'], 0, 4) == 'obj_'){
				import("lib.mongo.RQuery");
				$query = new RQuery($this->_mongo, $this->db, $this->collection);
				$id = new MongoDB\BSON\ObjectId(substr($j['_id'], 4));
	...
	public function doExplainQuery() {
		...
		if (!empty($queryHints)) {
			$db = $this->_mongo->selectDB($this->db);
			$indexes = $db->selectCollection($this->collection)->listIndexes();
		...
		//$cursor = $query->cursor();
		$explainContent = $query->explain();
		$this->ret = $this->_highlight($explainContent, "json");
		$this->display();
	}
...
window.parent.frames["left"].location.reload();
	...
	public function doCollectionIndexes() {
		...
		$this->indexes = [];
		$ret = $collection->listIndexes();
		foreach ($ret as $_index => $idxObj) {
			//TODO 这里的object和array的转换可能有问题, 需要更好的方法
			$index = mongo_object_to_array($idxObj);
	...
	public function doDeleteIndex() {
		...
		foreach ($indexes as $index) {
			if ($index["name"] == trim(xn("index"))) {
				$ret = $db->command(array("dropIndexes" => $collection->getCollectionName(), "index" => $index["name"]))->toArray()[0];
				break;
			}
		}
	...
	public function doCollectionStats() {
		...
		$db = $this->_mongo->selectDB($this->db);
		$ret = $db->command(["collStats" => $this->collection])->toArray()[0];
		if ($ret["ok"]) {
			$this->stats = $ret;
		...
		//top
		$ret = $this->_mongo->selectDB("admin")->command(["top" => 1])->toArray()[0];
		$this->top = array();
	...
	public function doCollectionValidate() {
		...
		$ret = $db->command(['validate'=>$this->collection])->toArray()[0];
	...
	public function doCollectionRename() {
		...
		if ($this->isPost()) {
			...
			$cmdParams = ['renameCollection'=>$this->db.'.'.$oldname, 'to'=>$this->db.'.'.$newname, 'dropTarget'=>(bool)$removeExists];
 			$this->ret = $this->_mongo->selectDB('admin')->command($cmdParams)->toArray()[0];
 			if ($this->ret["ok"]) {
