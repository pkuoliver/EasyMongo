...
class DbController extends BaseController {
	...
	public function doIndex() {
		...
		if(isset($ret["empty"])) $this->stats["Is Empty?"] = $ret["empty"] ? "Yes" : "No";
		...
		if (!$this->isPost()) {
			...
			foreach ($this->selectedCollections as $collectionName) {
				$cmd = array(
					...
					"from" => $this->_server->mongoHost() . ':' . $this->_server->mongoPort(),
					"copyIndexes" => (bool)$copyIndexes
				);
				$ret = $targetDb->command($cmd)->toArray()[0];
				if (!$ret["ok"]) {
	...
	public function doAuth() {
		...
		$this->users = $collection->find([])->toArray();
	...
	public function doNewCollection() {
		...
		if ($this->isPost()) {
			$db = $this->_mongo->selectDB($this->db);
			$options = ["capped" => boolval($this->isCapped)];
			if($this->size !== 0) $options['size'] = $this->size;
			if($this->max !== 0) $options['max'] = $this->max;

			MCollection::createCollection($db, $this->name, $options);
