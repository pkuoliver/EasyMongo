				...
				<tr>
					...
					<div class="operation" id="operate_<?php h($index);?>">
						...
						if ($this->last_format == "json") {
							$criteria = '{
	"files_id": ' . (($row["_id"] instanceof MongoDB\BSON\ObjectId) ? "ObjectId(\"" . addslashes($row["_id"]->__toString()) . "\")" : "\"" . addslashes($row["_id"]) . "\"") . '
}';
						} else {
							$criteria = 'array(
	"files_id" => ' . (($row["_id"] instanceof MongoDB\BSON\ObjectId) ? "new MongoDB\BSON\ObjectId(\"" . addslashes($row["_id"]->__toString()) . "\")" : "\"" . addslashes($row["_id"]) . "\"") . '
)';
						}
...
<table bgcolor="#cccccc" width="400" cellpadding="2" cellspacing="1">
	...
	<tr id="duplicate_tr" style="display:none" bgcolor="#ffffff">
		<td><?php rock_lang('removeduplicates'); ?>:</td>
		<td><?php rock_lang('duplicateskeyinfo'); ?> </td>
	</tr>
</table>
</div>
